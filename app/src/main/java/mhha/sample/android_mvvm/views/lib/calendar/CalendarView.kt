...
import mhha.sample.android_mvvm.databinding.CalendarViewBinding
...
import mhha.sample.android_mvvm.views.lib.calendar.adapters.*
import mhha.sample.android_mvvm.views.lib.calendar.exceptions.ErrorsMessages
import mhha.sample.android_mvvm.views.lib.calendar.exceptions.OutOfDateRangeException
    ...
    init {
        ...
        (relayCommand as RelayCommand).addEventListener(object: IEventListener {
            override fun onEvent(data: Any?) {
                ...
                when (data) {
                    "previousButton" -> binding.calendarViewPager.currentItem = binding.calendarViewPager.currentItem - 1
                    "forwardButton" -> binding.calendarViewPager.currentItem = binding.calendarViewPager.currentItem + 1
                    "ivClose" -> calendarProperties.onCloseClickListener?.onCloseClick()
                }
            }
        })
    }
...
//        LayoutInflater.from(context).inflate(R.layout.calendar_view, this)
    ...
    private fun initUiElements() {
        binding.previousButton.setOnClickListener {
            relayCommand.execute("previousButton")
        }

        binding.forwardButton.setOnClickListener {
            relayCommand.execute("forwardButton")
        }

        binding.ivClose.setOnClickListener {
            relayCommand.execute("ivClose")
        }
    }
